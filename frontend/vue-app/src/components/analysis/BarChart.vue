...
function render() { if (!element.value || !props.data.length) return; chart ||= echarts.init(element.value); chart.setOption({ tooltip: { trigger: 'axis' }, grid: { left: 42, right: 16, top: 32, bottom: 48 }, xAxis: { type: 'category', data: props.data.map((item) => item.name), axisLabel: { color: '#627d98', rotate: 22 } }, yAxis: { type: 'value', axisLabel: { color: '#627d98' }, splitLine: { lineStyle: { color: '#edf2f7' } } }, series: [{ name: props.title, type: 'bar', data: props.data.map((item) => item.value), barMaxWidth: 36, itemStyle: { color: '#2f80ed', borderRadius: [5, 5, 0, 0] } }] }) }
