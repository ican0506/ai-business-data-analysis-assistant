...
const emit = defineEmits(['clean', 'detail'])
...
<template>
  <el-card class="dataset-list-card" shadow="never">
    ...
    <el-table v-else :data="records" class="dataset-history-table">
      ...
      <el-table-column label="操作" width="210" fixed="right"><template #default="scope"><el-button link type="primary" @click="emit('detail', scope.row)">查看详情</el-button><el-button link type="success" :loading="cleaningId === scope.row.id" :disabled="scope.row.status === 'CLEANING'" @click="emit('clean', scope.row)">开始清洗</el-button></template></el-table-column>
    </el-table>
  </el-card>
</template>
