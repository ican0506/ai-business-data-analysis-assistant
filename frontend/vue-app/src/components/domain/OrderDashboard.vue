...
const kpis = computed(() => [
  ...
  overview.value.sales_total !== null && overview.value.sales_total !== undefined
    ? { label: '可信销售额', value: formatNumber(overview.value.sales_total), suffix: '', trend: '可信金额口径', type: 'success' } : null,
  overview.value.average_order_value !== null && overview.value.average_order_value !== undefined
    ? { label: '平均客单价', value: formatNumber(overview.value.average_order_value), suffix: '', trend: '有效销售订单', type: 'warning' } : null,
...
<template>
  ...
  <div class="dashboard-grid domain-dashboard-grid">
    <el-card v-if="orderAnalysis.product_analysis?.length" class="dashboard-panel" shadow="never"><template #header><strong>商品销售排行</strong></template><BarChart title="商品销售额" :data="orderAnalysis.product_analysis.map(item => ({ name: item.name, value: item.sales_amount }))" /></el-card>
    <el-card v-if="orderAnalysis.category_analysis?.length" class="dashboard-panel" shadow="never"><template #header><strong>品类销售排行</strong></template><BarChart title="品类销售额" :data="orderAnalysis.category_analysis.map(item => ({ name: item.category, value: item.sales_amount }))" /></el-card>
    <el-card v-if="metrics.top_regions?.length" class="dashboard-panel" shadow="never"><template #header><strong>地区销售表现</strong></template><BarChart title="地区销售额" :data="metrics.top_regions" /></el-card>
    <el-card v-if="timeAnalysis.monthly_sales_trend?.length" class="dashboard-panel" shadow="never"><template #header><strong>月度销售趋势</strong></template><LineChart title="月度销售额" :data="timeAnalysis.monthly_sales_trend" /></el-card>
    ...
    <el-card v-if="orderAnalysis.payment_method_analysis?.length" class="dashboard-panel" shadow="never"><template #header><strong>支付方式分布</strong></template><BarChart title="支付方式销售额" :data="orderAnalysis.payment_method_analysis.map(item => ({ name: item.name, value: item.sales_amount }))" /></el-card>
  </div>
  <div class="dashboard-grid domain-dashboard-grid">
