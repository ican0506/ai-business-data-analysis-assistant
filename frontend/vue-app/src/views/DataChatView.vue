...
import {
  clearDataChatMessages,
  clearSelectedDataChatDataset,
  loadDataChatMessages,
  loadSelectedDataChatDataset,
  saveDataChatMessages,
  saveSelectedDataChatDataset,
} from '../utils/dataChatStorage'
...
async function loadDatasets() {
  try {
    ...
    const savedDatasetId = loadSelectedDataChatDataset()
    const savedDataset = datasets.value.find((dataset) => dataset.id === savedDatasetId)
    if (savedDataset) activeDatasetId.value = savedDataset.id
    else if (savedDatasetId) clearSelectedDataChatDataset()
  } catch (error) {
    errorMessage.value = error.message || '数据集加载失败，请稍后重试。'
  }
}

watch(activeDatasetId, (datasetId) => {
  if (!datasetId) {
    messages.value = []
    return
  }
  saveSelectedDataChatDataset(datasetId)
  messages.value = loadDataChatMessages(datasetId)
})
...
<template>
  <section class="data-chat-view">
    ...
    <el-card shadow="never" class="dataset-card">
      ...
      <el-select id="chat-dataset" v-model="activeDatasetId" placeholder="请选择数据集" clearable :disabled="loading">
    ...
    <el-card shadow="never" class="chat-card">
      <div v-if="messages.length" class="chat-toolbar"><span>当前对话仅保存在本浏览器标签页中</span><el-button text type="danger" :disabled="loading" @click="clearConversation">清空对话</el-button></div>
