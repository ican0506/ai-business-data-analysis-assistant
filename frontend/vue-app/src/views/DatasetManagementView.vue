...
import { addDatasetRecord, loadDatasetHistory, setActiveDatasetId, updateDatasetRecord } from '../utils/datasetHistory'
...
function openAnalysis(dataset) { setActiveDatasetId(dataset.id); router.push({ name: 'dashboard' }) }
async function openMapping(dataset) { setActiveDatasetId(dataset.id); await analysisStore.load(dataset.id); mappingVisible.value = true }
