...
import { downloadDatasetReport, getDatasets } from '../api/datasets'
import ReportCard from '../components/download/ReportCard.vue'
import { useAuthStore } from '../stores/auth'
import { datasetLabel, toDatasetRecord } from '../utils/datasetHistory'
import { loadReportHistory, saveReportRecord } from '../utils/reportHistory'

const auth = useAuthStore()
const records = ref([])
const history = ref(loadReportHistory(auth.user?.id))
const downloadingKey = ref('')

const cards = computed(() => records.value.flatMap((dataset) => [
  { id: `${dataset.id}-excel`, datasetId: dataset.id, datasetName: datasetLabel(dataset), type: 'excel', reportName: `${dataset.fileName}-数据集#${dataset.id}-分析报告.xlsx`, description: '可编辑指标明细与业务摘要' },
  { id: `${dataset.id}-word`, datasetId: dataset.id, datasetName: datasetLabel(dataset), type: 'word', reportName: `${dataset.fileName}-数据集#${dataset.id}-分析报告.docx`, description: '适合汇报与业务复盘的文档报告' },
  { id: `${dataset.id}-pdf`, datasetId: dataset.id, datasetName: datasetLabel(dataset), type: 'pdf', reportName: `${dataset.fileName}-数据集#${dataset.id}-分析报告.pdf`, description: '适合留档与正式分发的固定版报告' },
]))

async function loadDatasets() { records.value = (await getDatasets()).map(toDatasetRecord) }
...
async function downloadReport(report) {
  ...
  try {
    ...
    history.value = saveReportRecord(auth.user?.id, report)
...
<template>
  <section class="download-center-view">
    <header class="download-intro"><div><p class="view-eyebrow">REPORT DELIVERY</p><h2>报告下载中心</h2><p>面向汇报、复盘与留档，一键导出 Excel、Word、PDF 三类实时业务报告。</p></div><el-tag type="info" effect="plain">当前用户数据集 + 本机轻量历史</el-tag></header>
